...
# creating different models based on different corpus to do comparison
model_2_path1,model_3_path1 = create_models(corpus_direction)
model_2_path2,model_3_path2 = create_models(corpus_direction2)

# we can also convert the models into csv files to understand the models
model_2_path1.to_csv('model_with_2_path1.csv', index=False)
model_3_path1.to_csv('model_with_3_path1.csv', index=False)
model_2_path2.to_csv('model_with_2_path2.csv', index=False)
model_3_path2.to_csv('model_with_3_path2.csv', index=False)
...
def get_word(previous_words, remaining, line, model_2, model_3):
    if len(previous_words) >= 2:
        ...
        if len(subset) == 0:
            return get_word([previous_words[-1]], remaining, line,model_2,model_3)
...
def generate_haiku(model_2, model_3):
    w = get_first_word(model_2)
    #print(w)
    previous_words = [w]
    haiku = [[w], [], []]
    ...
    for i,j in enumerate(counts):
        ...
        while remain > 0:
            try:
                w = get_word(previous_words, remain, i, model_2, model_3)
